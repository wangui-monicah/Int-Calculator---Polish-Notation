 ...
 * @author Wangui Monicah Wambugu || UNI: mww2137
...
public class IntCalculator {
    ...
    public String infixToPostfix() throws StackException {
        ...
        for (int i = 0; i < infixExpression.length(); i++) {
            ...
            if (isDigit(ch)) {
                ...
                while (!stack.isEmpty() && precedence(ch) < precedence(stack.peek()) ||
                        !stack.isEmpty() && precedence(ch) == precedence(stack.peek()) && isLeftAssociative(stack.peek())) {
                    if(!stack.isEmpty())postfixExpression.append(stack.pop());
                    postfixExpression.append(' ');
                }
                stack.push(ch);
            }
        ...
        if(postfixExpression.charAt(postfixExpression.length()-1) == ' '){
     ...
     */
     public int evaluatePostfix()
        ...
        for (int i = 0; i < postfixExpression.length(); i++) {
            ...
            char pre = ' '; //variable to store pre int values in multidigit number
            if(i - 1 > -1) {
                pre = postfixExpression.charAt(i - 1);
            }
            if (isDigit(ch)) {
                if(isDigit(pre)){
                    String initial = (stack.pop()).toString();
                    String post = String.valueOf(ch);
                    initial += post;
                    stack.push(Integer.parseInt(initial));
                }
                ...
                if(!stack.isEmpty()) {
                    operand2 = stack.pop();
                    if(!stack.isEmpty())operand1 = stack.pop();
                }
                if (ch == '^') {
